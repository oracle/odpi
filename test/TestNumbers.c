...
//-----------------------------------------------------------------------------
// Test_BindLargeUintAsNativeUint() [PRIVATE]
...
//-----------------------------------------------------------------------------
int Test_BindLargeUintAsNativeUint(dpiTestCase *testCase,
...
//-----------------------------------------------------------------------------
// Test_BindLargeUintAsOracleNumber() [PRIVATE]
//   Verify that a large unsigned integer (larger than can be represented by
// a signed integer) is transferred to Oracle and returned from Oracle
// successfully as an Oracle number.
//-----------------------------------------------------------------------------
int Test_BindLargeUintAsOracleNumber(dpiTestCase *testCase,
        dpiTestParams *params)
{
    uint32_t numQueryColumns, bufferRowIndex;
    const char *sql = "select :1 from dual";
...
//-----------------------------------------------------------------------------
// Test_FetchLargeUintAsNativeUint() [PRIVATE]
//   Verify that a large unsigned integer (larger than can be represented by
// a signed integer) can be fetched from Oracle successfully as a native
// unsigned integer.
//-----------------------------------------------------------------------------
int Test_FetchLargeUintAsNativeUint(dpiTestCase *testCase,
        dpiTestParams *params)
{
...
//-----------------------------------------------------------------------------
// Test_FetchLargeUintAsOracleNumber() [PRIVATE]
//   Verify that a large unsigned integer (larger than can be represented by
// a signed integer) can be fetched from Oracle successfully as an Oracle
// number.
//-----------------------------------------------------------------------------
int Test_FetchLargeUintAsOracleNumber(dpiTestCase *testCase,
        dpiTestParams *params)
{
...
int main(int argc, char **argv)
{
    ...
    dpiTestSuite_addCase(Test_BindLargeUintAsOracleNumber,
            "bind large unsigned integer as Oracle number");
    dpiTestSuite_addCase(Test_BindLargeUintAsNativeUint,
            "bind large unsigned integer as native unsigned integer");
    dpiTestSuite_addCase(Test_FetchLargeUintAsOracleNumber,
            "fetch large unsigned integer as Oracle number");
    dpiTestSuite_addCase(Test_FetchLargeUintAsNativeUint,
            "fetch large unsigned integer as native unsigned integer");

// add test cases for fetching as number and native
